...
use crate::fem::FEM;
use crate::fem::structs::{NODE_DOF, TRUSS_NODE_DOF, Truss};
    ...
    fn compose_error_message(&self) -> String {
        match self {
            ...
            Self::SameNodes(node_1_number, node_2_number) => {
                format!(
                    "Truss element with node number {node_1_number} and {node_2_number} already exists!"
                )
            }
            Self::NumberNotExist(number) => {
                format!("Truss element with number {number} does not exist!")
            }
        }
    }
}
    ...
    V: FloatTrait<Output = V>,
{
    ...
    pub fn add_truss(
        ...
        let local_stiffness_matrix =
            truss_element.extract_local_stiffness_matrix(self.get_nodes())?;
