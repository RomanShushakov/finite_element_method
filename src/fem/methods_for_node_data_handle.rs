    ...
    V: Debug,
{
    fn compose_error_message(&self) -> String {
        match self {
            ...
            NodeError::NumberNotExist(number) => {
                format!("Node with number {number} does not exist!")
            }
    ...
    V: FloatTrait<Output = V>,
{
    fn check_node_data(
        ...
        if node_index > self.get_props().get_nodes_number() as usize - 1 {
            return Some(NodeError::Limit(
                self.get_props().get_nodes_number() as usize
            ));
        }
    ...
    pub fn add_node(&mut self, number: u32, x: V, y: V, z: V) -> Result<(), String> {
        ...
        self.get_mut_index_node_number_map()
            .insert(node_index, number);
