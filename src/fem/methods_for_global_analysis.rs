...
fn convert_k_aa_into_compacted_form<V>(
    k_aa_matrix: &SquareMatrix<V>, k_aa_skyline: &Vec<usize>,
) 
    ...
    where V: FloatTrait<Output = V>
{
    pub fn find_ua_vector(
        &self, separated_stiffness_matrix: &SeparatedStiffnessMatrix<V>, r_a_vector: &Vector<V>, u_b_vector: &Vector<V>,
    ) 
        -> Result<Vector<V>, String>
    {
        let (mut a, maxa) = convert_k_aa_into_compacted_form(
            separated_stiffness_matrix.get_k_aa_matrix(), separated_stiffness_matrix.get_k_aa_skyline(),
        )?;

        let mut b = find_b(&r_a_vector, separated_stiffness_matrix.get_k_ab_matrix(), &u_b_vector)?;
    ...
    pub fn find_r_r_vector(
        &self, separated_stiffness_matrix: &SeparatedStiffnessMatrix<V>, u_a_vector: &Vector<V>, u_b_vector: &Vector<V>,
    ) 
        -> Result<Vector<V>, String>
    {
        let r_r = find_r_r(
    ...
    pub fn compose_global_analysis_result(
        &mut self, k_aa_indexes: &[usize], k_bb_indexes: &[usize], u_a_vector: &Vector<V>, r_r_vector: &Vector<V>,
    )
        -> Result<(), String>
    {
        for i in 0..k_aa_indexes.len()
        {
            ...
            *self.get_mut_displacements_vector().get_mut_element_value(&Position(index, 0))? = 
        ...
        for i in 0..k_bb_indexes.len()
        {
            ...
            *self.get_mut_forces_vector().get_mut_element_value(&Position(index, 0))? = 
