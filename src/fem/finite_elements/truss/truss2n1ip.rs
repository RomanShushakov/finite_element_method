        ...
        -> Result<Self, String>
    {
        ...
        for integration_point in integration_points.iter()
        {
            let matrix = TrussAuxFunctions::local_stiffness_matrix(
                node_1_number, node_2_number, young_modulus, area, area_2, integration_point.weight, 
                integration_point.r, &local_stiffness_matrix, nodes,
            )?;
            local_stiffness_matrix = matrix;
        }

        let mut nodes_dof_parameters =
            TrussAuxFunctions::<V>::compose_node_dof_parameters(node_1_number)?;

        let node_2_dof_parameters =
            TrussAuxFunctions::<V>::compose_node_dof_parameters(node_2_number)?;
...
impl<V> FiniteElementTrait<V> for Truss2n1ip<V>
    where V: FloatTrait<Output = V>,
{
    fn update(
        &mut self, 
    ...
    fn extract_stiffness_groups(&self) -> Vec<StiffnessGroup>
    {
        ...
        vec![
            StiffnessGroup { 
             ...
             StiffnessGroup { 
                stiffness_type: StiffnessType::Kuu, number_1: 
                self.node_1_number,
