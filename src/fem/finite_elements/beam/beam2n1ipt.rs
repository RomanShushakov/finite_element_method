             ...
             MulAssign + SubAssign + 'static
{
                       ...
                       -> ExtendedMatrix<T, V>
    {
        ...
        let length = BeamAuxFunctions::<T, V>::length(node_1, node_2).into();
    ...
    fn dx_dr(x_1: V, x_2: V, r: V) -> V
    {
        BeamAuxFunctions::<T, V>::derivative_x(
            BeamAuxFunctions::<T, V>::power_func_x, V::from(0.5) * x_1,
            V::from(0.0), 0) -
        BeamAuxFunctions::<T, V>::derivative_x(
            BeamAuxFunctions::<T, V>::power_func_x, V::from(0.5) * x_1, r, 1) +
        BeamAuxFunctions::<T, V>::derivative_x(
            BeamAuxFunctions::<T, V>::power_func_x, V::from(0.5) * x_2,
            V::from(0.0), 0) +
        BeamAuxFunctions::<T, V>::derivative_x(
            BeamAuxFunctions::<T, V>::power_func_x, V::from(0.5) * x_2, r, 1)
    }


    fn jacobian(node_1: Rc<RefCell<FENode<T, V>>>, node_2: Rc<RefCell<FENode<T, V>>>, r: V) -> V
    {
        let length = BeamAuxFunctions::length(node_1, node_2);
    ...
    fn inverse_jacobian(node_1: Rc<RefCell<FENode<T, V>>>, node_2: Rc<RefCell<FENode<T, V>>>, r: V)
                        -> V
    {
        V::from(1.0) / BeamAuxFunctions::jacobian(node_1, node_2, r)
    }
    ...
    {
        BeamAuxFunctions::jacobian(node_1, node_2, r)
    }
    ...
    {
        BeamAuxFunctions::<T, V>::derivative_x(
            BeamAuxFunctions::<T, V>::power_func_x, V::from(0.5), V::from(0.0), 0) -
        BeamAuxFunctions::<T, V>::derivative_x(
            BeamAuxFunctions::<T, V>::power_func_x, V::from(0.5), r, 1)
    }
    ...
    {
        let elements = vec![BeamAuxFunctions::<T, V>::dh1_dr(r), V::from(0.0),
                            V::from(0.0), BeamAuxFunctions::<T, V>::dh2_dr(r), V::from(0.0), V::from(0.0)];
        let mut matrix = ExtendedMatrix::create(T::from(1),
            T::from(TRUSS2N2IP_NODES_NUMBER * TRUSS_NODE_DOF), elements);
        let inverse_jacobian = BeamAuxFunctions::inverse_jacobian(node_1, node_2, r);
    ...
    {
        let current_area = BeamAuxFunctions::<T, V>::area(area_1, area_2, r);
        let mut lhs_matrix =
            ...
            Ok(mut matrix) =>
                {
                    matrix.multiply_by_number(
                        BeamAuxFunctions::determinant_of_jacobian(
             ...
             MulAssign + SubAssign + 'static
{
                  ...
                  -> Result<Self, String>
    {
        ...
        let rotation_matrix =
            BeamAuxFunctions::rotation_matrix(Rc::clone(&node_1),
                                              Rc::clone(&node_2));

        rotation_matrix.show_matrix();
        println!();

        let shape = rotation_matrix.get_shape();
        ...
        local_n_1_direction.show_matrix();
        println!();
        ...
        let mut nodes_dof_parameters =
            BeamAuxFunctions::<T, V>::compose_node_dof_parameters(
        ...
        Ok(Beam2n1ipT { number, node_1, node_2, young_modulus, area, area_2, state })
    }
             ...
             PartialEq + Debug + Default + 'static,
{
    ...
    {
        let rotation_matrix =
            BeamAuxFunctions::rotation_matrix(Rc::clone(&self.node_1),
        ...
        -> Result<ElementAnalysisData<T, V>, String>
    {
        ...
        if self.area_2.is_some()
        {
            ...
            for ip in &self.state.integration_points
            {
                ...
                {
                    let area = BeamAuxFunctions::<T, V>::area(
            ...
            for point_number in 0..POINTS_NUMBER_FOR_TAPERED_TRUSS
            {
                ...
                let area = BeamAuxFunctions::<T, V>::area(self.area, self.area_2, r);
