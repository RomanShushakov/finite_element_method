...
use crate::fem::quadrilateral_4n_element_functions::{
    convex_hull_on_four_points_on_plane, determinant_of_jacobian_at_r_s, dh_dx_dh_dy,
    extract_transformed_directions_of_nodes, find_rotation_matrix_elements_of_quadrilateral,
    h1_r_s, h2_r_s, h3_r_s, h4_r_s, is_points_of_quadrilateral_on_the_same_line,
    ...
    V: FloatTrait<Output = V>,
{
    ...
    Ok(Matrix::create(
        ...
        &[
            ...
            (V::from(1f32) + s) * V::from(-1f32) * (y_1 - y_2) / V::from(4f32)
                * gamma_rz_multiplier,
            ...
            V::from(0f32),
            V::from(-1f32) * (V::from(1f32) - s) / V::from(2f32) * gamma_rz_multiplier,
            (V::from(1f32) - s) * V::from(-1f32) * (y_4 - y_3) / V::from(4f32)
                * gamma_rz_multiplier,
            ...
            (V::from(1f32) + r) * V::from(-1f32) * (y_1 - y_4) / V::from(4f32)
                * gamma_sz_multiplier,
            ...
            V::from(0f32),
            (V::from(1f32) - r) / V::from(2f32) * gamma_sz_multiplier,
            (V::from(1f32) - r) * V::from(-1f32) * (y_2 - y_3) / V::from(4f32)
                * gamma_sz_multiplier,
    ...
    V: FloatTrait<Output = V>,
{
    ...
    let c_mem = Matrix::create(
        ...
        &[
            ...
            V::from(0f32),
            V::from(0f32),
            (V::from(1f32) - poisson_ratio) / V::from(2f32),
        ],
    )
    ...
    V: FloatTrait<Output = V>,
{
    ...
    for i in 0..PLATE_NODES_NUMBER {
        *local_stiffness_matrix.get_mut_element_value(&Position(
            i * PLATE_NODE_DOF + PLATE_NODE_DOF - 1,
            i * PLATE_NODE_DOF + PLATE_NODE_DOF - 1,
        ))? += V::from(KROT6);
    }
    ...
    V: FloatTrait<Output = V>,
{
    pub fn create(
        ...
        let integration_points = [
            (
                V::from(1f32 / 3f32).my_sqrt() * V::from(1f32),
                V::from(1f32 / 3f32).my_sqrt() * V::from(1f32),
                V::from(1f32),
                V::from(1f32),
            ),
    ...
    fn is_node_belongs_to_element(&self, node_number: u32) -> bool {
        self.node_1_number == node_number
            || self.node_2_number == node_number
            || self.node_3_number == node_number
            || self.node_4_number == node_number
    }

    pub fn is_nodes_numbers_same(&self, nodes_numbers: &[u32; 4]) -> bool {
        nodes_numbers
            .iter()
            .all(|node_number| self.is_node_belongs_to_element(*node_number))
    }
    ...
    pub fn convert_uniformly_distributed_surface_load_to_nodal_loads(
        ...
        let distributed_force_matrix =
            Matrix::create(1, 1, &[uniformly_distributed_surface_load_value]);
    ...
    pub fn extract_element_analysis_result(
        ...
        let node_2_index = nodes
            ...
            .get_index();
        let node_3_index = nodes
            .get(&self.node_3_number)
            .ok_or(format!("Node: {} is absent!", self.node_3_number))?
            .get_index();
        let node_4_index = nodes
            .get(&self.node_4_number)
            .ok_or(format!("Node: {} is absent!", self.node_4_number))?
            .get_index();
        let mut global_displacements =
            Vector::create(&[V::from(0f32); PLATE_NODES_NUMBER * PLATE_NODE_DOF]);
        ...
        let c_mem = Matrix::create(
            ...
            &[
                ...
                V::from(0f32),
                V::from(0f32),
                (V::from(1f32) - self.poisson_ratio) / V::from(2f32),
            ],
        )
        .multiply_by_scalar(c_multiplier_mem);
        ...
        for (r, s) in local_nodes_coordinates.iter() {
            ...
            strain_displacement_matrix_mem =
                strain_displacement_matrix_mem.add(&strain_displacement_matrix_mem_at_r_s)?;
        }
        ...
        for (r, s) in local_nodes_coordinates.iter() {
            ...
            strain_displacement_matrix_bend =
                strain_displacement_matrix_bend.add(&strain_displacement_matrix_bend_at_r_s)?;
        }
        let element_strains_bend =
            strain_displacement_matrix_bend.multiply(&local_displacements)?;
        let element_forces_bend = c_bend
            .multiply(&element_strains_bend)?
            .multiply_by_scalar(self.thickness.my_powi(2) / V::from(24f32));

        let c_multiplier_shear =
        ...
        for (r, s) in local_nodes_coordinates.iter() {
            ...
            strain_displacement_matrix_shear =
                strain_displacement_matrix_shear.add(&strain_displacement_matrix_shear_at_r_s)?;
        }
        let element_strains_shear =
            strain_displacement_matrix_shear.multiply(&local_displacements)?;
        let element_forces_shear = c_shear
            .multiply(&element_strains_shear)?
            .multiply_by_scalar(
                self.thickness * self.shear_factor / V::from(local_nodes_coordinates.len() as f32),
            );
