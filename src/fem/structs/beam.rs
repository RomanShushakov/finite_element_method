...
use crate::fem::bar_2n_element_functions::{
    determinant_of_jacobian_at_r, dh1_dr, dh2_dr, find_2n_element_vector, h1_r, h2_r,
    inverse_jacobian_at_r,
};
    ...
    fn compose_error_message(&self) -> String {
        match self {
            ...
            Self::PoissonRatio(value) => {
                format!("Poisson's ratio {value:?} is less or equal to zero!")
            }
            ...
            Self::ParallelLocalAxis(number, x, y, z) => {
                format!(
                    "Local axis 1 direction [{x:?}, {y:?}, {z:?}] parallel to element {number}!"
                )
            }
        }
    }
    ...
    V: FloatTrait<Output = V>,
{
    ...
    let projection_of_beam_section_orientation =
        local_axis_1_direction_vector.projection_perpendicular_to_vector(&beam_element_vector);
...
where
    V: FloatTrait<Output = V>,
{
    let mut angle = if i11 != i22 {
        (V::from(2f32) * i12 / (i22 - i11)).my_atan() / V::from(2f32)
    } else {
        let (i11_mod, i22_mod) = if i22 < i11 {
            (
                i11,
                (i22.my_abs() - i22.my_abs() * rel_tol) * i22 / i22.my_abs(),
            )
        } else {
            (
                (i11.my_abs() - i11.my_abs() * rel_tol) * i11 / i11.my_abs(),
                i22,
            )
        };
    ...
    let mut i11_p = i11 * (angle.my_cos()).my_powi(2) + i22 * (angle.my_sin()).my_powi(2)
    ...
    let mut i = 1;
    while i11_p < i22_p {
        angle = ((V::from(2f32) * i12 / (i22 - i11)).my_atan() + V::from(PI) * V::from(i as f32))
            / V::from(2f32);
        i11_p = i11 * (angle.my_cos()).my_powi(2) + i22 * (angle.my_sin()).my_powi(2)
            - i12 * (V::from(2f32) * angle).my_sin();
    ...
    V: FloatTrait<Output = V>,
{
    ...
    let direction_vector = Vector3::create(&[beam_element_length, V::from(0f32), V::from(0f32)]);

    let interim_rotation_matrix = beam_element_vector.rotation_matrix_to_align_with_vector(
        &direction_vector,
        rel_tol,
        abs_tol,
    )?;
    ...
    let transformed_projection_of_beam_section_orientation =
        interim_rotation_matrix.multiply(&projection_of_beam_section_orientation)?;

    let angle_between_beam_section_local_axis_1_direction_and_axis_t =
        Vector3::create(&[V::from(0f32), V::from(0f32), V::from(1f32)])
    ...
    let r_11 = if c_xz != V::from(0f32) {
        c_x
    } else {
        V::from(0f32)
    };
    let r_12 = c_y;
    let r_13 = if c_xz != V::from(0f32) {
        c_z
    } else {
        V::from(0f32)
    };
    let r_21 = if c_xz != V::from(0f32) {
        (V::from(-1f32) * c_x * c_y * c - c_z * s) / c_xz
    } else {
        V::from(-1f32) * c_y * c
    };
    let r_22 = if c_xz != V::from(0f32) {
        c_xz * c
    } else {
        V::from(0f32)
    };
    let r_23 = if c_xz != V::from(0f32) {
        (V::from(-1f32) * c_y * c_z * c + c_x * s) / c_xz
    } else {
        s
    };
    let r_31 = if c_xz != V::from(0f32) {
        (c_x * c_y * s - c_z * c) / c_xz
    } else {
        c_y * s
    };
    ...
    V: FloatTrait<Output = V>,
{
    ...
    let k_u_at_ip = b_u_t_at_r.multiply(&b_u_at_r)?.multiply_by_scalar(
        c_u * determinant_of_jacobian_at_r(node_1_number, node_2_number, r, nodes)? * alpha,
    );
    ...
    let k_v_at_ip = b_v_t_at_r.multiply(&b_v_at_r)?.multiply_by_scalar(
        c_v * determinant_of_jacobian_at_r(node_1_number, node_2_number, r, nodes)? * alpha,
    );
    ...
    let k_w_at_ip = b_w_t_at_r.multiply(&b_w_at_r)?.multiply_by_scalar(
        c_w * determinant_of_jacobian_at_r(node_1_number, node_2_number, r, nodes)? * alpha,
    );
    ...
    let k_thu_at_ip = b_thu_t_at_r.multiply(&b_thu_at_r)?.multiply_by_scalar(
        c_thu * determinant_of_jacobian_at_r(node_1_number, node_2_number, r, nodes)? * alpha,
    );
    ...
    let k_thv_at_ip = b_thv_t_at_r.multiply(&b_thv_at_r)?.multiply_by_scalar(
        c_thv * determinant_of_jacobian_at_r(node_1_number, node_2_number, r, nodes)? * alpha,
    );
    ...
    let k_thw_at_ip = b_thw_t_at_r.multiply(&b_thw_at_r)?.multiply_by_scalar(
        c_thw * determinant_of_jacobian_at_r(node_1_number, node_2_number, r, nodes)? * alpha,
    );
...
where
    V: FloatTrait,
{
    let [r_11, r_12, r_13, r_21, r_22, r_23, r_31, r_32, r_33] = rotation_matrix_elements;

    SquareMatrix::create(
        BEAM_NODES_NUMBER * BEAM_NODE_DOF,
        &[
            [*r_11, *r_12, *r_13],
            [V::from(0f32); BEAM_NODE_DOF / 2],
            [V::from(0f32); BEAM_NODE_DOF / 2],
            [V::from(0f32); BEAM_NODE_DOF / 2],
            [*r_21, *r_22, *r_23],
            [V::from(0f32); BEAM_NODE_DOF / 2],
    ...
    V: FloatTrait<Output = V>,
{
    ...
    pub fn convert_uniformly_distributed_line_load_to_nodal_loads(
        ...
        let distributed_force_matrix =
            Matrix::create(1, 1, &[uniformly_distributed_line_load_value]);

        let mut nodal_forces = Vector::create(&[V::from(0f32); 2]);
        for (r, alpha) in self.integration_points.iter() {
            let determinant_of_jacobian_at_r =
                determinant_of_jacobian_at_r(self.node_1_number, self.node_2_number, *r, nodes)?;
    ...
    pub fn extract_element_analysis_result(
        ...
        let mut global_displacements =
            Vector::create(&[V::from(0f32); BEAM_NODES_NUMBER * BEAM_NODE_DOF]);
        ...
        for (r, _alpha) in self.integration_points.iter() {
            ...
            strain_displacement_matrix_u =
                strain_displacement_matrix_u.add(&strain_displacement_matrix_u_at_r)?;
        }
        ...
        let shear_modulus =
            self.young_modulus / (V::from(2f32) * (V::from(1f32) + self.poisson_ratio));
        ...
        for (r, _alpha) in self.integration_points.iter() {
            ...
            strain_displacement_matrix_v =
                strain_displacement_matrix_v.add(&strain_displacement_matrix_v_at_r)?;
        }
        ...
        let element_forces_v = element_strains_v.multiply_by_scalar(
            shear_modulus * self.area * self.shear_factor
                / V::from(self.integration_points.len() as f32),
        );
        ...
        for (r, _alpha) in self.integration_points.iter() {
            ...
            strain_displacement_matrix_w =
                strain_displacement_matrix_w.add(&strain_displacement_matrix_w_at_r)?;
        }
        ...
        for (r, _alpha) in self.integration_points.iter() {
            ...
            strain_displacement_matrix_thu =
                strain_displacement_matrix_thu.add(&strain_displacement_matrix_thu_at_r)?;
        }
        ...
        let beam_element_vector =
            find_2n_element_vector(self.node_1_number, self.node_2_number, nodes)?;
        let beam_element_length = beam_element_vector.norm()?;
        ...
        for (r, _alpha) in self.integration_points.iter() {
            ...
            strain_displacement_matrix_thv =
                strain_displacement_matrix_thv.add(&strain_displacement_matrix_thv_at_r)?;
        }
        ...
        let moment_s_at_node_1 =
            moment_s_average + beam_element_length * force_t_value / V::from(2f32);
        let moment_s_at_node_2 =
            moment_s_average - beam_element_length * force_t_value / V::from(2f32);
        ...
        for (r, _alpha) in self.integration_points.iter() {
            ...
            strain_displacement_matrix_thw =
                strain_displacement_matrix_thw.add(&strain_displacement_matrix_thw_at_r)?;
        }
        ...
        let moment_t_at_node_1 =
            moment_t_average + beam_element_length * force_s_value / V::from(2f32);
        let moment_t_at_node_2 =
            moment_t_average - beam_element_length * force_s_value / V::from(2f32);
        ...
        let element_analysis_data = vec![
            ...
            (ElementForceComponent::ForceS, force_s_value),
            (ElementForceComponent::ForceT, force_t_value),
            (
                ElementForceComponent::MomentR,
                *element_forces_thu.get_element_value(&Position(0, 0))?,
            ),
            (ElementForceComponent::MomentS, moment_s_at_node_1),
            (ElementForceComponent::MomentS, moment_s_average),
            (ElementForceComponent::MomentS, moment_s_at_node_2),
            (ElementForceComponent::MomentT, moment_t_at_node_1),
            (ElementForceComponent::MomentT, moment_t_average),
            (ElementForceComponent::MomentT, moment_t_at_node_2),
        ];
